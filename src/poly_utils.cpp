...
//   return products
// }
typedef std::vector<std::vector<AltBn128_FrElement>> FrElementMatrix;

// 두 유한 필드 배열의 텐서 곱을 계산하는 함수
FrElementMatrix tensorProduct(const std::vector<AltBn128_FrElement>& array1, const std::vector<AltBn128_FrElement>& array2) {
    size_t numRows = array1.size();
    size_t numCols = array2.size();
    FrElementMatrix result(numRows, std::vector<AltBn128_FrElement>(numCols)); // 결과 저장을 위한 2차원 벡터 초기화

    for (size_t i = 0; i < numRows; ++i) {
        for (size_t j = 0; j < numCols; ++j) {
            Fr_mul(&result[i][j], &array1[i], &array2[j]); // 두 원소의 곱셈
        }
    }
